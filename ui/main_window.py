...
QMenu ,QApplication ,QGraphicsDropShadowEffect ,QGraphicsOpacityEffect ,QSizePolicy ,
QMessageBox )
...
from ui .animated_widgets import AnimatedTypingIndicator
from ui .responsive_layout import QuickActionBar ,StatusBar
from ui .quick_actions import SlashCommandMenu ,SmartSuggestions ,QuickReply ,WorkspaceContextBar ,CommandProcessor
...
class MainWindow (QMainWindow ):
    ...
    def __init__ (self ,user_data ):
        ...
        self .workspace_focus_file =None
        ...
        self .history_panel_width =320
        self .command_processor =CommandProcessor (self )
        self .message_animations =[]
        self .history_animation =None
        self .input_update_timer =QTimer (self )
        self .input_update_timer .setSingleShot (True )
        self .input_update_timer .setInterval (120 )
        self .input_update_timer .timeout .connect (self .process_message_input_update )
    ...
    def ensure_capabilities (self ):
        if self .capabilities is None :
            self .capabilities =KaiCapabilities (self .project_root )
    ...
    def create_sidebar (self ,parent ):
        ...
        inline_new_chat_btn =QPushButton ("+ Fresh Conversation")
        inline_new_chat_btn .setObjectName ("newChatButton")
        inline_new_chat_btn .clicked .connect (self .create_new_conversation )
        inline_new_chat_btn .setStyleSheet (f"""
            QPushButton {{
                background: {Config .ACCENT_GRADIENT };
        ...
        sidebar_layout .addWidget (inline_new_chat_btn )
        ...
        self .history_animation =QPropertyAnimation (sidebar ,b"maximumWidth")
        self .history_animation .setDuration (220 )
        self .history_animation .setEasingCurve (QEasingCurve .InOutCubic )
        self .history_animation .valueChanged .connect (lambda value : sidebar .setMinimumWidth (int (value )))
    ...
    def create_chat_area (self ,parent ):
        ...
        self .quick_action_bar .action_triggered .connect (self .handle_quick_action )
        chat_layout .addWidget (self .quick_action_bar )

        self .smart_suggestions =SmartSuggestions ()
        self .smart_suggestions .suggestion_selected .connect (self .apply_suggestion )
        chat_layout .addWidget (self .smart_suggestions )

        self .quick_reply_bar =QuickReply ()
        self .quick_reply_bar .reply_selected .connect (self .apply_suggestion )
        chat_layout .addWidget (self .quick_reply_bar )

        self .workspace_context_bar =WorkspaceContextBar ()
        self .workspace_context_bar .chip_selected .connect (self .handle_workspace_chip )
        chat_layout .addWidget (self .workspace_context_bar )
        ...
        self .slash_menu =SlashCommandMenu (chat_area )
        self .slash_menu .command_selected .connect (self .on_slash_command_selected )
    ...
    def toggle_history_panel (self ):
        self .history_panel_open =not self .history_panel_open
        target_width =self .history_panel_width if self .history_panel_open else 0
        if self .history_animation :
            self .history_animation .stop ()
            self .history_animation .setStartValue (self .history_panel .maximumWidth ())
            self .history_animation .setEndValue (target_width )
            self .history_animation .start ()
        else :
            self .history_panel .setMinimumWidth (target_width )
            self .history_panel .setMaximumWidth (target_width )
        self .history_btn .setText ("Hide History" if self .history_panel_open else "History")

    def update_responsive_layout (self ):
        if self .width ()<980 and self .history_panel_open :
            self .history_panel_open =False

        target_width =self .history_panel_width if self .history_panel_open and self .width ()>=980 else 0
        if self .history_animation and self .history_animation .state ():
            self .history_animation .stop ()
        self .history_panel .setMinimumWidth (target_width )
        self .history_panel .setMaximumWidth (target_width )
        self .history_btn .setText ("Hide History" if target_width else "History")

    def handle_quick_action (self ,action_type ,value ):
        if action_type =='command':
            self .command_processor .process (value )
        elif action_type =='reaction':
            self .add_message_widget ('ai',f"{value} Noted. I'll keep shaping this experience to feel fast, warm, and easy to use.",datetime .now ())

    def on_message_input_changed (self ,text ):
        self .pending_input_text =text
        self .input_update_timer .start ()

    def process_message_input_update (self ):
        text =self .pending_input_text
        self .smart_suggestions .update_suggestions (text )
        file_candidate =self .extract_project_file_candidate (text )
        if file_candidate :
            self .update_workspace_context (file_candidate )
        if text .startswith ('/'):
            self .slash_menu .filter_commands (text )
            menu_x =max (20 ,self .input_frame .x ()+20 )
            menu_y =max (80 ,self .input_frame .y ()-self .slash_menu .sizeHint ().height ()-8 )
            self .slash_menu .show_at (QPoint (menu_x ,menu_y ))
        else :
            self .slash_menu .hide ()

    def on_slash_command_selected (self ,command ,args ):
    ...
    def handle_workspace_chip (self ,command_text ):
        self .message_input .setText (command_text )
        self .execute_inline_command (command_text )

    def extract_project_file_candidate (self ,text ):
        if not text :
            return None

        capabilities =self .ensure_capabilities ()
        ...
        for candidate in candidates :
            ...
            if not normalized :
                continue
            context =capabilities .get_file_context (normalized )
            if 'error' not in context :
                return normalized
        return None

    def update_workspace_context (self ,file_path =None ):
        if not hasattr (self ,'workspace_context_bar'):
            return

        capabilities =self .ensure_capabilities ()

        focus_file =file_path or self .workspace_focus_file
        if not focus_file :
            self .workspace_context_bar .hide ()
            return

        context =capabilities .get_file_context (focus_file )
        if 'error' in context :
            self .workspace_context_bar .hide ()
            return

        self .workspace_focus_file =focus_file
        related_files =capabilities .get_related_files (focus_file )
        recent_files =[path for path in capabilities .get_recent_files ()if path !=focus_file ]
        self .workspace_context_bar .update_context (focus_file ,related_files ,recent_files )

    def execute_inline_command (self ,message_text ):
        parts =message_text .split ()
        command =parts [0 ]
        args =parts [1 :]
        handled =self .command_processor .process (command ,args )
        if handled :
            self .message_input .clear ()
            self .send_btn .setEnabled (True )
            if hasattr (self ,'status_bar_widget'):
                self .status_bar_widget .update_connection (True )
        else :
            self .add_message_widget ('ai',f"I don't know the command `{command }` yet.",datetime .now ())

    def add_message_widget (self ,sender ,content ,timestamp ,animate =True ,scroll =True ):
        message_widget =EnhancedMessageWidget (sender ,content ,timestamp )


        self .messages_layout .insertWidget (self .messages_layout .count ()-1 ,message_widget )
        if animate :
            self .animate_message_widget (message_widget )
        else :
            message_widget .setMaximumHeight (16777215 )


        if scroll :
            QTimer .singleShot (100 ,self .scroll_to_bottom )

    def animate_message_widget (self ,message_widget ):
        opacity_effect =QGraphicsOpacityEffect (message_widget )
        opacity_effect .setOpacity (0.0 )
        message_widget .setGraphicsEffect (opacity_effect )
        target_height =max (message_widget .sizeHint ().height (),60 )
        ...
        height_animation =QPropertyAnimation (message_widget ,b"maximumHeight")
        height_animation .setDuration (260 )
        ...
        self .message_animations .append ((opacity_animation ,height_animation ))

        def cleanup_animation ():
            message_widget .setMaximumHeight (16777215 )
            try :
                self .message_animations .remove ((opacity_animation ,height_animation ))
            except ValueError :
                pass

        height_animation .finished .connect (cleanup_animation )
        opacity_animation .start ()
        height_animation .start ()

    def show_typing_indicator (self ):
        if self .typing_indicator is None :
            self .typing_indicator =AnimatedTypingIndicator ()
    ...
    def send_message (self ):
        ...
        file_candidate =self .extract_project_file_candidate (message_text )
        if file_candidate :
            self .update_workspace_context (file_candidate )

        if message_text .startswith ('/'):
            self .execute_inline_command (message_text )
            return

        if hasattr (self ,'status_bar_widget'):
            self .status_bar_widget .connection_label .setText ("🟡 Thinking")
        if hasattr (self ,'quick_reply_bar'):
            self .quick_reply_bar .hide ()
    ...
    def on_ai_response_received (self ,ai_response ,metadata ):
        try :
            ...
            if hasattr (self ,'status_bar_widget'):
                self .status_bar_widget .update_connection (True )
                self .status_bar_widget .update_credits (credits_remaining )
                self .status_bar_widget .model_label .setText (f"Model: {display_model }")
            ...
            file_candidate =self .extract_project_file_candidate (ai_response )
            if file_candidate :
                self .update_workspace_context (file_candidate )
            if hasattr (self ,'quick_reply_bar'):
                self .quick_reply_bar .update_replies (ai_response )
    ...
    def clear_messages (self ):
        ...
        if hasattr (self ,'quick_reply_bar'):
            self .quick_reply_bar .hide ()
        if hasattr (self ,'workspace_context_bar'):
            self .workspace_context_bar .hide ()
        self .workspace_focus_file =None
    ...
    def load_conversation (self ,conversation_id ):
        try :
            ...
            if messages :
                ...
                for msg in messages :
                    self .add_message_widget (msg ['sender'],msg ['content'],msg ['timestamp'],animate =False ,scroll =False )

                last_file_candidate =None
                for msg in reversed (messages ):
                    last_file_candidate =self .extract_project_file_candidate (msg ['content'])
                    if last_file_candidate :
                        break
                if last_file_candidate :
                    self .update_workspace_context (last_file_candidate )
