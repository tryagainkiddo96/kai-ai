...
class TextFormatter:
    ...
    def format_with_multiple_styles(content, sender='ai'):
        ...
        styled_lines = []
        for raw_line in formatted_content.split('\n'):
            stripped = raw_line.strip()
            if not stripped:
                styled_lines.append('<div style="height: 8px;"></div>')
                continue

            if re.match(r'^\d+\.\s+', stripped):
                styled_lines.append(
                    f'<div style="margin: 4px 0; color: {base_color}; line-height: 1.5;">{stripped}</div>'
                )
            elif stripped.startswith('- '):
                styled_lines.append(
                    f'<div style="margin: 4px 0; color: {base_color}; line-height: 1.5;">&#8226; {stripped[2:]}</div>'
                )
            elif stripped.endswith(':') and len(stripped) <= 48:
                styled_lines.append(
                    f'<div style="margin: 8px 0 4px 0; color: {bold_color}; font-weight: 700; letter-spacing: 0.2px;">{stripped}</div>'
                )
            else:
                styled_lines.append(
                    f'<div style="margin: 2px 0; color: {base_color}; line-height: 1.5;">{stripped}</div>'
                )
